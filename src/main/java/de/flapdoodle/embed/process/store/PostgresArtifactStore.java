...
/**
 * @author Ilya Sadykov Hacky ArtifactStore. Just to override the default
 *         FilesToExtract with PostgresFilesToExtract
 */
public class PostgresArtifactStore implements IMutableArtifactStore {
	...
	PostgresArtifactStore(IDownloadConfig downloadConfig, IDirectory tempDirFactory, ITempNaming executableNaming,
	...
	public void removeFileSet(Distribution distribution, IExtractedFileSet all) {
		...
		if (all.generatedBaseDir() != null && !deleteQuietly(all.generatedBaseDir())) {
			LOGGER.trace("Could not delete generatedBaseDir: {}", all.generatedBaseDir());
		}
	}
	...
	public boolean checkDistribution(Distribution distribution) throws IOException {
		if (!LocalArtifactStore.checkArtifact(downloadConfig, distribution)) {
			return LocalArtifactStore.store(downloadConfig, distribution,
					downloader.download(downloadConfig, distribution));
		}
		return true;
	}
