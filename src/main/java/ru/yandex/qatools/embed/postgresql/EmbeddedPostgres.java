...
public class EmbeddedPostgres {
	...
	private static final List<String> DEFAULT_ADD_PARAMS = asList("-E", "SQL_ASCII", "--locale=C", "--lc-collate=C",
			"--lc-ctype=C");
	...
	/**
	 * Initializes the default runtime configuration using the temporary
	 * directory.
	 *
	 * @return runtime configuration required for postgres to start.
	 */
	public static IRuntimeConfig defaultRuntimeConfig() {
		return new RuntimeConfigBuilder().defaults(Command.Postgres)
				.artifactStore(new PostgresArtifactStoreBuilder().defaults(Command.Postgres)
						.download(new PostgresDownloadConfigBuilder().defaultsForCommand(Command.Postgres).build()))
				.build();
	}

	/**
	 * Initializes runtime configuration for cached directory. If a provided
	 * directory is empty, postgres will be extracted into it.
	 *
	 * @param cachedPath
	 *            path where postgres is supposed to be extracted
	 * @return runtime configuration required for postgres to start
	 */
	public static IRuntimeConfig cachedRuntimeConfig(File cachedPath) {
		final Command cmd = Command.Postgres;
		final FixedPath cachedDir = new FixedPath(cachedPath.getPath());
		return new RuntimeConfigBuilder().defaults(cmd)
				.artifactStore(new PostgresArtifactStoreBuilder().defaults(cmd).tempDir(cachedDir)
						.download(new PostgresDownloadConfigBuilder().defaultsForCommand(cmd)
								.packageResolver(new PackagePaths(cmd, cachedDir)).build()))
				.build();
	}
	...
	public String start(IRuntimeConfig runtimeConfig) throws IOException {
		return start(runtimeConfig, DEFAULT_HOST, findFreePort(), DEFAULT_DB_NAME, DEFAULT_USER, DEFAULT_PASSWORD,
				DEFAULT_ADD_PARAMS);
	}
	 ...
	 *            additional database init params (if required)
	 * @return connection url for the initialized postgres instance
	 * @throws IOException
	 *             if an I/O error occurs during the process startup
	 */
	public String start(IRuntimeConfig runtimeConfig, String host, int port, String dbName, String user,
			String password, List<String> additionalParams) throws IOException {
		final PostgresStarter<PostgresExecutable, PostgresProcess> runtime = PostgresStarter.getInstance(runtimeConfig);
		config = new PostgresConfig(version, new AbstractPostgresConfig.Net(host, port),
				new AbstractPostgresConfig.Storage(dbName, dataDir), new AbstractPostgresConfig.Timeout(),
	...
	public Optional<String> getConnectionUrl() {
		...
		if (postgresConfig != null) {
			...
			return fromNullable(formatConnUrl);
		} else {
	...
	public void stop() {
		Optional<PostgresProcess> process = getProcess();
		if (process.isPresent()) {
			process.get().stop();
		} else {
			throw new IllegalStateException("Cannot stop not started instance!");
		}
	}
}
